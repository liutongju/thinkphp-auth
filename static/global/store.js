;(function(root, factory){
    ...
    var storage = window.localStorage,store,_api,even_storage=function(){},even_before_storage=function(){return true;};
    ...
    _api = {
        set: function(key, val){
            ...
            var ov=this.get(key);
            var rv=even_before_storage('set',key,val,ov);
            if(!(!!rv||typeof(rv)==void 0)){return false;}
            storage.setItem(key, stringify(val));
            even_storage('set',key,val,ov);
            return this;
        },
        ...
        clear: function(){
        	notdel=[];
            this.forEach(function(key, val) {
            	var rv=even_before_storage('clear',key,null,val);
	            if(!(!!rv||typeof(rv)==void 0)){
	            	notdel.push({key:key,val:val});
	            }
                even_storage('clear',key,null,val);
            });
            storage.clear();
            var len=notdel.length,i=0;
            while (i<len){
            	var o=notdel[0];
            	storage.setItem(o.key,stringify(o.val));
            	i++;
            };
            return this;
        },
        remove: function(key) {
            ...
            var rv=even_before_storage('remove',key,null,val);
	        if(!(!!rv||typeof(rv)==void 0)){
	            return false;
	        }
            storage.removeItem(key);
            even_storage('remove',key,null,val);
            return this;
        },
        ...
        onBeforeStorage: function(cb){
            if(cb && isFunction(cb)) even_before_storage = cb;
            return this;
        }
    };
